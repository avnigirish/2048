...
public class Board {
    ...
    public void updateOpenSpaces() {
        ...
        for(int x=0; x<gameBoard.length; x++){
            for(int y=0; y<gameBoard[x].length; y++){
                if(gameBoard[x][y]==0){
                    openSpaces.add(new BoardSpot(x,y));
                }
            }
        }
    }
    ...
    public void addRandomTile() {
        ...
        if(v<0.1)
            gameBoard[openSpaces.get(t).getRow()][openSpaces.get(t).getCol()] = 4;
        else
            gameBoard[openSpaces.get(t).getRow()][openSpaces.get(t).getCol()] = 2;
    }
    ...
    public void swipeLeft() {
        // WRITE YOUR CODE HERE
        for(int x=0; x<gameBoard.length; x++){
            for(int y=1; y<gameBoard[0].length; y++){
                int index=y;         
                if(gameBoard[x][y-1]==0){
                    try{
                        while(gameBoard[x][index-1]==0){
                            gameBoard[x][index-1] = gameBoard[x][index];
                            gameBoard[x][index] = 0;
                            index--;    
                        }
                    }catch (Exception e){}     
    ...
    public void mergeLeft() {
        ...
        for(int x=0; x<gameBoard.length; x++){
            for(int y=1; y<gameBoard[0].length; y++){
                if(gameBoard[x][y-1]==gameBoard[x][y]){
                    gameBoard[x][y-1]*=2;
                    gameBoard[x][y]=0;
                }
            }
    ...
    public void flipRows() {
        ...
        for(int x=0; x<gameBoard.length; x++){
            for(int y=0; y<gameBoard[0].length; y++){
                temp[x][y] = gameBoard[x][(gameBoard[0].length-1)-y];
            }
        }
        gameBoard=temp;
    }
